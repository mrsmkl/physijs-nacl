...
var Physijs = (function() {
	...
	getEulerXYZFromQuaternion = function ( x, y, z, w ) {
		return new THREE.Vector3(
            ...
            Math.asin( Math.max(-1.0, Math.min(1.0, 2 *  ( x * z + y * w )) )),
	...
	Physijs.Scene = function( params ) {
		...
		this._worker = physModule; // new Worker( Physijs.scripts.worker || 'physijs_worker.js' );
		this._objects = [];
		...
		this._worker.onmessage = function ( event ) {
			...
			if (data && data.cmd) switch ( data.cmd ) {
				
				case 'update':
                    // console.log(data.params.objects.length);
					for ( obj_id in data.params.objects ) {
						if ( !data.params.objects.hasOwnProperty( obj_id ) ) continue;
						
						obj = data.params.objects[obj_id];
                    ...
                    data.params.collisions0.forEach(function (id, i) {
                        sceneObj = self._objects[id];
                        collisionObj = self._objects[data.params.collisions1[i]];
                    ...
                    for (var i in self._objects) {
                        if ( !self._objects.hasOwnProperty(i) ) continue;
                        self._objects[i]._physijs.touches.length = 0;
                    }
					
                    data.params.collisions0.forEach(function (id, i) {
                        sceneObj = self._objects[id];
                        var id2 = data.params.collisions1[i];

						if ( sceneObj._physijs.touches.indexOf( id2 ) === -1 ) {
							sceneObj._physijs.touches.push( id2 );
						}
                    });
	...
	Physijs.Scene.prototype.execute = function( cmd, params ) {
		this._worker.postMessage(JSON.stringify({ cmd: cmd, params: params }));
	};
	...
	Physijs.Scene.prototype.remove = function( object ) {
		...
		if ( object._physijs ) {
            // console.log("Removing: " + object._physijs.id);
